<?php
  ...
  */
   ...
   * @copyright CiviCRM LLC (c) 2004-2013
   *
   * PUM Issue 1979 - add select for drupal roles
   * @author Erik Hommel (CiviCooP) <erik.hommel@civicoop.org>
   *
   */
class CRM_Civireports_Form_Report_RoleActivities extends CRM_Report_Form {
    ...
    foreach ($this->_columns as $tableName => $table) {
      if (array_key_exists('filters', $table)) {
        ...
        foreach ($table['filters'] as $fieldName => $field) {
          if (!isset($field['pseudofield']) || $field['pseudofield'] == 0) {
            $clause = NULL;
...
GROUP BY civicrm_activity_id {$this->_having} {$this->_orderBy} {$this->_limit}";
  ...
  /**
   * Method to retrieve alle Drupal roles
   *
   * @access private
   */
  ...
  /**
   * Method to add a where clause for the Drupal roles if any where selected
   *
  ...
  private function addDrupalRoleWhereClause() {
    if (isset($this->_filters['civicrm_contact']['user_role'])) {
      ...
      $clause = null;
      $contactIds = array();
      if (!empty($this->_params['user_role_value'])) {
  ...
  /**
   * Method to get all contacts for a selected role
   *
   * @param int $userRoleId
   * @param array $contactIds
   * @access protected
   */
  private function getUsersForRole($userRoleId, &$contactIds) {
    if (!empty($userRoleId)) {
      $fetchedUsers = db_select('users_roles', 'ur')
        ->fields('ur', array('uid'))
        ->condition('ur.rid', $userRoleId)
        ->execute()
        ->fetchCol();
